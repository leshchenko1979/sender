...
@pytest.mark.parametrize("crontab, last_run, now, expected, _id", [
    ("0 * * * *", datetime(2021, 1, 1, 10, 30), datetime(2021, 1, 1, 11, 1), True, "ID001"),
    ("*/15 * * * *", datetime(2021, 1, 1, 10, 45), datetime(2021, 1, 1, 11, 0), True, "ID002"),
    ...
    ("0 12 1 * *", datetime(2021, 1, 1, 12, 0), datetime(2021, 1, 1, 12, 1), False, "ID006")
])
...
# Edge cases
@pytest.mark.parametrize("crontab, last_run, now, expected, _id", [
    ("0 * * * *", datetime(2021, 1, 1, 10, 59), datetime(2021, 1, 1, 11, 0), True, "ID007"),
    ...
    ("0 0 1 1 *", datetime(2021, 12, 31, 23, 59), datetime(2022, 1, 1, 0, 0), True, "ID010")
])
...
# Error cases
@pytest.mark.parametrize("crontab, last_run, now, expected_exception, _id", [
    ("not a cron", datetime(2021, 1, 1), datetime(2021, 1, 1, 1), ValueError, "ID011"),
...
@pytest.mark.parametrize("crontab, last_run, now, expected, _id", [
    ("0 * * * *", datetime(2021, 1, 1, 10, 59), datetime(2021, 1, 1, 11, 0), False, "ID007"),
    ("0 0 29 2 *", datetime(2020, 2, 29), datetime(2024, 2, 29), True, "ID008.1"),
    ("0 0 1 1 *", datetime(2021, 12, 31, 23, 59), datetime(2022, 1, 1, 0, 0), False, "ID010")
])
...
@pytest.mark.parametrize("crontab, tz, last_run, now, expected, _id", [
    ("0 9 * * *", Moscow, datetime(2021, 1, 1, 8, 30, tzinfo=UTC), datetime(2021, 1, 1, 9, 30, tzinfo=UTC), False, "ID001"),
    ("0 14 * * *", Moscow, datetime(2024, 1, 14, 10, 30, tzinfo=UTC), datetime(2024, 1, 14, 11, 30, tzinfo=UTC), True, "ID002"),
])
